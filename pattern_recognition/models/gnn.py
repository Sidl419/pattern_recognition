...
from torch_geometric.nn import GINConv, Sequential, TopKPooling
...
class FlexGCN(nn.Module):
    def __init__(
        ...
        for _ in range(num_layers):
            ...
            layers.append(
                (
                    GINConv(nn.Linear(input_feat_dim, output_feat_dim)),
                    "x, edge_index -> x",
                )
            )
            ...
            layers.append(
                (
                    TopKPooling(output_feat_dim, 0.5),
                    "x, edge_index, None, batch -> x, edge_index, _, batch, _, _",
                )
            )
            input_feat_dim = output_feat_dim
...
class BigGIN(nn.Module):
    def __init__(self, in_features, out_features, hidden, adj, eps=0.0):
    ...
    def forward(
        ...
        adj = self.adj + (1 + self.eps) * torch.eye(
            self.adj.shape[0], device=input.device
        )
...
class SmallGIN(nn.Module):
    def __init__(self, in_features, out_features, adj, eps=0.0):
...
class BaseGNN(nn.Module):
    def __init__(
        ...
        self.conv = nn.Conv1d(
            channel_filters, 1, kernel_size=time_kernel, padding="same"
        )
...
class GIN(nn.Module):
    def __init__(self, in_features, out_features, eps=0.0):

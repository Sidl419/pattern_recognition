...
def get_random_sample(dataset, learning_params, label=1):
    if learning_params["model_type"] == "GNN":
        ...
        sample.y = (
            (torch.arange(0, learning_params["num_classes"]) == label)
            .float()
            .unsqueeze(0)
        )
...
def generate_sample(
    model, init_sample, criterion, learning_params, num_steps=1001, device="cpu"
):
    model.eval()
...
def get_best_samples(
    model, dataloader, learning_params, top_n=4, label=1, device="cpu"
):
    model.eval()
    response_list = []
    label_list = []
    ...
    return (
        response_ind[:top_n],
        response_list[response_ind[:top_n]],
        label_list[response_ind[:top_n]],
    )

import pytest
from pattern_recognition.data.pipelines import get_pipeline
from pattern_recognition.selections.packing import collate_selection_packets
from torch.utils.data import DataLoader


@pytest.mark.parametrize(
    ("protocol", "expect_rowcol"),
    [
        ("bci3_rowcol", True),
        ("samara_single_flash_sim", False),
    ],
)
def test_synthetic_selection_packets_collate(protocol, expect_rowcol):
    pipe = get_pipeline("SyntheticSelectionPackets")(
        protocol=protocol,
        n_train=4,
        n_val=2,
        n_channels=1,
        n_times=64,
        r_max=2,
        seed=0,
    )
    bundle = pipe.build()
    loader = DataLoader(
        bundle.train, batch_size=2, collate_fn=collate_selection_packets
    )
    batch = next(iter(loader))
    assert batch["epochs"].ndim == 4
    assert batch["flash_targets"].shape[:2] == batch["epochs"].shape[:2]
    if expect_rowcol:
        assert "row_target" in batch and "column_target" in batch
    else:
        assert "row_target" not in batch
        assert batch["stimulus_codes"][batch["valid_mask"]].min() >= 1
        assert batch["character_target"].shape == (2,)
